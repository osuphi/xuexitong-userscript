(function () {
    ...
    //
    // 设计边界（V3.5 起）：
    //   * 本仓库不含任何自动作答功能：视频内的互动题弹窗只做检测并提示人工完成；
    //     每节课末尾的章节测验/作业一律直接跳过（保留有限次数上限，用于防止页面循环）；
    //   * 全脚本不写 localStorage、不落盘、不外发任何凭据；
    //   * 任何配置下都不会发起外部网络请求（唯一外链是页面缺 jQuery 时补加载的 CDN）；
    ...
    function initializePlayer() {
        const app = {
            ...
            configs: {
                ...
                playTimeoutMs: 8000,
                // F20（本地修复）：恢复播放的次数上限；**显式设为 0 表示不限次数**。
                resumeMaxAttemptsPerUnit: 5,
                // F20（本地修复）：是否"尊重用户主动暂停"。
                //   false（本仓库默认）= 无论谁暂停（用户点的 / 页面防挂机 / 浏览器省电），都按有界策略自动恢复；
                //                        需要真正停下时，把油猴里的本脚本关掉即可。
                //   true            = 上游原行为：用户点过暂停就不再抢播
                //                        （上游为避免无差别 play() 触发风控而选择的方式，见 #26 #32 #54 #55）。
            ...
            _resumeCap() {
                // F20（本地修复）：恢复播放的次数上限；configs.resumeMaxAttemptsPerUnit 设为 0 表示"不限次数"。
                const cap = Number(this.configs.resumeMaxAttemptsPerUnit);
                if (cap === 0) return Infinity;
                if (!Number.isFinite(cap) || cap < 0) return 5; // 缺省或非法值 → 保守默认
                return cap;
            },
            _tryResumePlayback(reason) {
                ...
                const resumeCap = this._resumeCap();
                if (this._resumeAttemptsThisUnit >= resumeCap) {
                    if (!this._resumeCapLogged) {
                        this._resumeCapLogged = true;
                        console.warn(`%c本节恢复播放次数已达上限（${Number.isFinite(resumeCap) ? resumeCap : '不限'} 次），停止抢播（#32 #54 #55）。`, 'color:#FF9800');
                ...
                console.log(`%c触发视频保活恢复(${reason}) ${this._resumeAttemptsThisUnit}/${Number.isFinite(resumeCap) ? resumeCap : '∞'}`, 'color:#607D8B');
            ...
            _scheduleGuardProbe(reason) {
                ...
                if (this._resumeAttemptsThisUnit >= resumeCap) {
                        ...
                        if (!this._resumeCapLogged) {
                            this._resumeCapLogged = true;
                            console.warn(`%c本节恢复/回拨次数已达上限（${Number.isFinite(resumeCap) ? resumeCap : '不限'} 次），停止保活动作（#32 #54 #55）。`, 'color:#FF9800');
                        }
                        return;
                    }
            ...
            _handleVideoPause(e) {
                ...
                // F20（本地修复）：只有显式把 respectUserPause 设为 true 才沿用上游"尊重用户暂停"的行为；
                // 默认 false —— 无论谁暂停都会自动恢复（想真正停下就把本脚本关掉）。
                const userIntent = this._isUserPauseIntent(now);
                if (userIntent && this.configs.respectUserPause === true) {
                    this._userPaused = true;
                    console.warn('%c检测到用户主动暂停：按配置不再自动恢复播放（#26 #32 #55）。', 'color:#FF9800');
                    console.log('处理方法：需要恢复自动保活时，请点击播放按钮、执行 app.resumeAutoPlay()，'
                        + '或把 app.configs.respectUserPause 设为 false 后 app.run()。');
                    return;
                }
                if (userIntent) {
                    console.log('%c检测到用户主动暂停，但当前配置为「始终自动恢复」：稍后会继续播放（想真正停下请关闭本脚本）。', 'color:#FF9800');
                    return;
                }
            ...
            _checkInteractionDialog() {
                ...
                if (found) {
                    // 命中互动题弹窗：一律暂停自动跳转并等人工完成（本仓库不含任何自动作答）。
                    if (!this._interactionBlocked) {
            ...
            _guiInit() {
                ...
                // 拖动补丁：本次页面会话里如果拖过，面板就沿用上次位置（run() 重建面板时不会跳回右上角）
                if (this._guiPos && Number.isFinite(this._guiPos.left) && Number.isFinite(this._guiPos.top)) {
                    const w = this._guiPos.width || 330;
                    const h = this._guiPos.height || 160;
                    const maxLeft = Math.max(0, (window.innerWidth || 0) - w);
                    const maxTop = Math.max(0, (window.innerHeight || 0) - Math.min(h, window.innerHeight || h));
                    panel.style.left = Math.min(Math.max(0, this._guiPos.left), maxLeft) + 'px';
                    panel.style.top = Math.min(Math.max(0, this._guiPos.top), maxTop) + 'px';
                    panel.style.right = 'auto';
                }
                const header = document.createElement('div');
                // 拖动补丁：标题栏兼作拖动手柄（禁用文本选择与触摸滚动，避免拖的时候选中文字/滚动页面）
                ...
                // ===================== 拖动补丁（本地修改，非上游代码） =====================
                // 按住标题栏即可把面板拖到任意位置；点标题栏右侧的折叠按钮不会触发拖动。
                // 位置只记在内存里，刷新后回到右上角，与本脚本"不写 localStorage"的约定保持一致。
                let dragging = false;
                ...
                const onDragDown = (e) => {
                    if (e.pointerType === 'mouse' && e.button !== 0) return; // 只认鼠标左键
                    if (e.target && e.target.closest && e.target.closest('button')) return; // 不抢折叠按钮的点击
                    const rect = panel.getBoundingClientRect();
                    dragging = true;
                    dragPointerId = e.pointerId;
                    dragOffsetX = e.clientX - rect.left;
                    dragOffsetY = e.clientY - rect.top;
                    // 面板原本是 right/top 定位，拖动必须切到 left/top，否则两个方向会互相打架
                ...
                const onDragMove = (e) => {
                    ...
                    const pos = clampToViewport(e.clientX - dragOffsetX, e.clientY - dragOffsetY, w, h);
                ...
                header.addEventListener('pointerup', onDragUp);
                header.addEventListener('pointercancel', onDragUp);
                this._guiDragHandlers = { header: header, down: onDragDown, move: onDragMove, up: onDragUp };
                console.log('%c[GUI] 面板已支持拖动：按住标题栏移动（位置仅在本次页面会话内有效）', 'color:#607D8B');
                // =========================== 拖动补丁结束 ===========================
            ...
            _guiDestroy() {
                ...
                // 拖动补丁：销毁时一并摘掉拖动监听，保持"destroy() 后页面零残留监听"的既有约定
                if (this._guiDragHandlers) {
                    const d = this._guiDragHandlers;
                    try {
                        d.header.removeEventListener('pointerdown', d.down);
                        d.header.removeEventListener('pointermove', d.move);
                        d.header.removeEventListener('pointerup', d.up);
                        d.header.removeEventListener('pointercancel', d.up);
                    } catch (e) { /* ignore */ }
                    this._guiDragHandlers = null;
                }
            ...
            _guiRefreshStatus(force) {
                ...
                try {
                    ...
                    lines.push('状态: ' + (this._isPlaying ? '播放中' : '空闲')
                        ...
                        + ' ｜ 恢复策略: ' + (this.configs.respectUserPause === true
                            ? (this._userPaused ? '尊重暂停（已暂停）' : '尊重暂停')
                            : '始终自动'));
        ...
        window[APP_KEY] = app;
        // 油猴带 @grant 时脚本运行在沙箱中，
        // 补一条 unsafeWindow 桥，保证页面控制台里的 app.run() 等调试流程仍可用。
        try {
